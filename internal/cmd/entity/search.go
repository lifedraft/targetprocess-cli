...
func newSearchCmd(f *cmdutil.Factory) *cli.Command {
  ...
  tp entity search --type UserStory -w 'entityState.isFinal!=true' -s 'id,name,entityState.name as state'

  # Cross-type search
  tp entity search --type Assignable -w 'name.toLower().contains("login")' -s 'id,name,entityType.name as type'

  # Use a preset
  tp entity search --type UserStory --preset open

  # With sorting
  tp entity search --type Bug -w 'priority.name=="High"' --order-by 'createDate desc' --take 50

  # High priority unassigned items
  tp entity search --type Assignable --preset highPriorityUnassigned -s 'id,name,entityType.name as type'

  # Recently modified items
  tp entity search --type Assignable --preset recentActivity`,
		Flags: []cli.Flag{
			cmdutil.OutputFlag(),
			&cli.StringFlag{
				Name:     "type",
				Required: true,
				Usage:    `Entity type (e.g. UserStory, Bug, Task, Feature). Use "Assignable" for cross-type search`,
			},
			&cli.StringFlag{
				Name:    "where",
				Aliases: []string{"w"},
			...
			&cli.StringFlag{
				Name:  "preset",
				Usage: "Use a preset filter (run 'tp entity presets' to list available presets)",
			},
			&cli.StringFlag{
				Name:    "select",
				Aliases: []string{"s"},
			...
			&cli.StringFlag{
				Name:  "include",
				Usage: "Legacy alias for --select (backward compatibility with v1)",
			},
			&cli.IntFlag{
				Name:    "take",
				Aliases: []string{"t"},
		...
		Action: func(ctx context.Context, cmd *cli.Command) error {
			...
			// --include is a legacy alias for --select (backward compat)
			if inc := cmd.String("include"); inc != "" && selectExpr == "" {
				selectExpr = inc
			}
			...
			if presetName := cmd.String("preset"); presetName != "" {
				...
				where = p.Where
				// Use preset's select/orderBy only if not explicitly set by user
...
func printV2EntityTable(w io.Writer, entities []api.Entity) {
	...
	// Detect which columns are available from the first entity
...
// getField retrieves a field from an entity, trying camelCase first (v2) then PascalCase (v1).
...
// getNestedName retrieves a nested name field, handling both v2 flat aliases and v1 nested objects.
func getNestedName(e api.Entity, aliasKey, nestedObjKey string) any {
	...
	// v2-style nested object with camelCase
...
// detectColumns determines table columns based on the keys present in the entity.
func detectColumns(sample api.Entity) []column {
	var cols []column

	// Always try to show ID
	...
	// Additional dynamic columns for v2 aliased fields not yet covered
